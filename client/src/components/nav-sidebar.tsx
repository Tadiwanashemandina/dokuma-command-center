...
/**
 * Administrative items, appended after the modules.
 *
 * The audit trail is deliberately NOT admin-only: it is oversight rather than
 * administration, and migration 0013's policy granted it to admin, exec and
 * finance_manager. The Express route enforces the same three.
 */
...
export function NavSidebar({ role }: { role: UserRole }) {
  ...
  const items = [...NAV_ITEMS, ...ADMIN_ITEMS].filter((item) => !item.roles || item.roles.includes(role));
