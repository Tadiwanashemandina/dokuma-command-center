...
import { computeWeeklyReportData } from "@/lib/finance/reports";
import { createWeeklyReportAction } from "../../actions";
...
export default async function NewWeeklyReportPage({
  ...
  const data = await computeWeeklyReportData(periodStart, periodEnd);

  return (
    <div className="max-w-3xl space-y-6">
      <div>
        <h1 className="font-serif text-3xl font-semibold text-navy">New Weekly Report</h1>
      ...
      <form className="space-y-6">
        ...
        <Card className="rounded-2xl bg-muted/40">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-steel">5. Comparison with Previous Weeks</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2 p-4 pt-0 text-sm">
            {data.trend.map((t) => (
        ...
        <div className="space-y-2">
          <Label htmlFor="next_week_plan">10. Next Week Plan</Label>
          <Textarea id="next_week_plan" name="next_week_plan" required rows={3} />
        </div>

        <div className="flex gap-3">
          <Button type="submit" name="action" value="draft" formAction={createWeeklyReportAction} variant="outline">
            Save as Draft
          </Button>
          {canPublish && (
            <Button type="submit" name="action" value="publish" formAction={createWeeklyReportAction} className="bg-navy hover:bg-navy/90">
              Save &amp; Publish
            </Button>
          )}
        </div>
